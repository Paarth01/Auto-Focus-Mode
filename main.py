...
if __name__ == "__main__":
    ...
    # Check if GUI should be launched (default: yes, unless --no-gui flag is used)
    launch_gui = "--no-gui" not in sys.argv
    ...
    if launch_gui:
        ...
        try:
            ...
            # Launch GUI in a separate process to avoid blocking
            import subprocess
            gui_path = Path(__file__).parent / "gui" / "gui_main.py"
            if gui_path.exists():
                print("[INFO] Launching GUI with auto-start...")
                # Start GUI process with auto-start flag (it will start the daemon automatically)
                subprocess.Popen([sys.executable, "-m", "gui.gui_main", "--auto-start"], 
                               cwd=Path(__file__).parent)
                print("[INFO] GUI launched. Focus mode will start automatically.")
                print("[INFO] To run without GUI, use: python3 main.py --no-gui")
                # Exit - GUI will manage the daemon
                sys.exit(0)
        except ImportError:
            print("[WARNING] PyQt6 not available. Running in CLI mode only.")
            print("[INFO] Install PyQt6 to use the GUI: pip install PyQt6")
        except Exception as e:
            print(f"[WARNING] Failed to launch GUI: {e}")
            print("[INFO] Falling back to CLI mode.")
    
    # CLI mode: run daemon directly
    try:
